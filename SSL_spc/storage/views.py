...
def direct_Str(request):
    ...
    if request.user.is_authenticated:
        ...
        for temp in flnames:
            ...
            print("tem"+temp)
            if num == -1:
                url ='''http://127.0.0.1:8000/storage/files?filename='''+urllib.parse.quote(temp,safe='')
                flname=tempst
                dict[flname]=url
            else:
                tempst2 = tempst[:num]
                if tempst2 not in vfls:
                    url='''http://127.0.0.1:8000/storage/direct_Str/?directn='''+urllib.parse.quote(strt+tempst2+'/', safe='')
                    dirname=tempst2
                    dict[dirname]=url
                    vfls.append(tempst2)
                else:
        ...
        context["dict"]=dict;
        context["title"]=title
        template = loader.get_template('web_client/directory.html')
