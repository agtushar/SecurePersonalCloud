...
def direct_Str(request):
    if request.user.is_authenticated:
        ...
        for temp in flnames:
            ...
            print("tem" + temp)
            if num == -1:
                htst = '''<li><a href="http://127.0.0.1:8000/storage/files?filename=''' + urllib.parse.quote(temp,
                                                                                                             safe='') + '''">''' + tempst + "</a></li>"
                tempht = html + htst
                html = tempht
            else:
                ...
                if tempst2 not in vfls:
                    htst = '''<li><a href="http://127.0.0.1:8000/storage/direct_Str/?directn=''' + urllib.parse.quote(
                        strt + tempst2 + '/', safe='') + '''">''' + tempst2 + "</a></li>"
                    tempht = html + htst
                    html = tempht
                    vfls.append(tempst2)
                else:
    ...
    #     else:
    #         title=strt
    #     html = "<html lang=\"en\"><head><meta charset=\"ISO-8859-1\"><title>"+title+"</title><link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css\" integrity=\"sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u\" crossorigin=\"anonymous\"><script src=\"https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js\"></script><script src=\"https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js\"></script></head><body><div class=\"container\"><p>This is the current list of files</p> <ul> Files"
    #     vfls = []
    #     for temp in flnames:
    #         tempst = temp[len(strt):]
    #         num = tempst.find('/')
    #         print("tem"+temp)
    #         if num == -1:
    #             htst = '''<li><a href="http://127.0.0.1:8000/storage/files?filename='''+urllib.parse.quote(temp,safe='')+'''><button type=\"button\" class=\"btn btn-primary\">'''+tempst+'''</button></a></li>'''
    #             tempht = html+htst
    #             html = tempht
    #         else:
    #             tempst2 = tempst[:num]
    ...
    #             else:
    #                 pass
    #         tempht = html + "</ul></div></body>"
    #         html = tempht
    # else:
    #     html = "<body><div><p>You need to login to perform this operation</p></div></body>"
    # return HttpResponse(html)
